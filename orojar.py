...
def first_directional_derivative(G, z,  x, G_z, epsilon, path_idx=None, path_matrix=None):
    """
    Computes the first directional derivative of G w.r.t. its input at z in the direction x or w.
    """
    if path_idx is None:  # Apply the OroJaR in Z-space
        return (G(z + x,  path_matrix=path_matrix) - G_z) / epsilon

    else:  # Apply it in W-space
        return (G(z,  path_idx=path_idx+x, path_matrix=path_matrix) - G_z) / epsilon
...
def orojar(G, z, path_idx=None, G_z=None, k=2, epsilon=0.1, reduction=torch.mean,
    ...
    for i in range(k):
        x = xs[i]
        if multiple_layers:
            central_first_order = multi_layer_first_directional_derivative(G, z,  x, G_z, epsilon, path_idx=path_idx, path_matrix=path_matrix)
        else:
            central_first_order = first_directional_derivative(G, z,  x, G_z, epsilon, path_idx=path_idx, path_matrix=path_matrix)
