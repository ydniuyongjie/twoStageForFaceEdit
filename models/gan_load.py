...
########################################################################################################################
##                                                                                                                    ##
##                                                  [ StyleGAN2 for hessian_penalty]                                  ##
##                                                                                                                    ##
########################################################################################################################
class StyleGAN2Wrapper_hessian(nn.Module):
    def __init__(self, G, shift_in_w_space):
        super(StyleGAN2Wrapper_hessian, self).__init__()
        self.G = G
        self.shift_in_w_space = shift_in_w_space
        self.dim_z = 512
        self.dim_w = self.G.style_dim if self.shift_in_w_space else self.dim_z
    ...
    def forward(self, z , shift=None,path_idx=None , path_matrix=None, latent_is_w=False):
        """StyleGAN2 generator forward function.
        ...
        """
        # The given latent codes lie on Z- or W-space, while the given shifts lie on the W-space
        if path_idx is None and path_matrix is None:
            if shift is None:
                shift=None
        else:
            shift = path_matrix(path_idx)   
...
def build_stylegan2_hessian(pretrained_gan_weights, resolution, shift_in_w_space=False):
    # Build StyleGAN2 generator model
    G = StyleGAN2Generator(resolution, 512, 8)
    # Load pre-trained weights
    G.load_state_dict(torch.load(pretrained_gan_weights)['g_ema'], strict=False)

    return StyleGAN2Wrapper_hessian(G, shift_in_w_space=shift_in_w_space)
