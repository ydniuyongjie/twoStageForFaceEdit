...
def main():
    ...
    parser.add_argument('-g', '--gan-type', type=str, required=True, choices=GAN_WEIGHTS.keys(),help='GAN generator model type')
    ...
    for i in range(args.num_samples):
        ...
        z = zs[i, :].unsqueeze(0)
        latent_code_hash = sha1(z.cpu().numpy()).hexdigest()#返回摘要，作为十六进制数据字符串值
