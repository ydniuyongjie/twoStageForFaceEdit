...
def main():
    ...
    parser.add_argument('--shift-steps',
                        type=int,
                        default=16,
                        help="number of shifts per positive/negative path "
                        "direction")
    ...
    if not osp.isdir(args.exp):
        ...
        if not osp.isfile(args_json_file):
            raise FileNotFoundError(
                "File not found: {}".format(args_json_file))
    ...
    if (args.shift_steps is None) and (args.eps is None):
        latent_space_traversal_configs = [
            dI for dI in os.listdir(latent_traversal_dir)
            if osp.isdir(os.path.join(latent_traversal_dir, dI))
        ]
    else:
        latent_space_traversal_configs = [
            '{}_{}_{}'.format(2 * args.shift_steps, args.eps,
                              round(2 * args.shift_steps * args.eps, 3))
        ]

    if args.verbose:
        print("#. Calculate attribute traversals in {}".format(
            latent_traversal_dir))
        print("  \\__.Latent space traversal configs: {}".format(
            latent_space_traversal_configs))
    ...
    fairface.load_state_dict(
        torch.load(
            'models/pretrained/fairface/fairface_alldata_4race_20191111.pt'))
    fairface.eval()
    if use_cuda:
        fairface.cuda()
    ...
    for l_config in latent_space_traversal_configs:
        if args.verbose:
            print("       \\__.Latent space traversal config: {}".format(
                l_config))
        ...
        hashes_dir = osp.join(
            ...
            '{}_{}_{}'.format(2 * args.shift_steps, args.eps,
                              round(2 * args.shift_steps * args.eps, 3)))
        hashes = [
            dI for dI in os.listdir(hashes_dir)
            if osp.isdir(os.path.join(hashes_dir, dI)) and dI not in (
                'paths_gifs', 'validation_results')
        ]
        ...
        for h in hashes:
            ...
            paths_latent_codes = torch.load(osp.join(h_dir, 'paths_latent_codes.pt'),
                map_location=lambda storage, loc: storage)  #(200,33,512)
            ...
            for d in range(num_of_paths):
                if args.verbose:
                    update_progress("               \\__path: {:03d}/{:03d} ".format(d + 1, num_of_paths), num_of_paths, d + 1)
                ...
                data_loader = data.DataLoader(
                    dataset=PathImages(root_path=osp.join(
                        path_images_dir, 'path_{:03d}'.format(d))),
                ...
                if gan_type == 'StyleGAN2':
                    ...
                    with torch.no_grad():
                        attribute_predictions = celeba_5(
                            celeba_5_trans(
                                path_images_tensor.div(255.0).mul(2.0).add(
                                    -1.0)))
                else:
                ...
                for attr, attr_predictions in attribute_predictions.items():
                    attr_scores = celeba_5_softmax(
                        attr_predictions).cpu().numpy()
                    ...
                    if attr == 'Bangs':
                        ...
                        celeba_eyeglasses_dict.update(
                            {d: final_scores.tolist()})
                    elif attr == 'No_Beard':
                ...
                #       x = x[:, :, 35:223, 32:220])
                path_images_tensor_resized = face_detector_trans(
                    path_images_tensor)
                original_img = path_images_tensor_resized[
                    num_of_img_per_path // 2, :].unsqueeze(0)
                id_scores = [
                    id_comp(
                ...
                for t in range((num_of_img_per_path - 1) // 2):
                    transformed_img = path_images_tensor_resized[
                        num_of_img_per_path // 2 + t + 1, :].unsqueeze(0)
                    with torch.no_grad():
                        id_sim = id_comp(
                            original_img.div(255.0).mul(2.0).add(-1.0),
                ...
                for t in range(len(detected_faces)):
                    cropped_faces[t] = face_trans_hope_fair(
                        crop_face(
                            ...
                            idx=t,
                            bbox=detected_faces[t][0][:-1] if
                            len(detected_faces[t]) > 0 else [0, 0, 256, 256],
                ...
                for t in range(len(detected_faces)):
                    cropped_faces[t] = face_trans_hope_fair(
                        crop_face(
                            ...
                            if len(detected_faces[t]) > 0 else
                            [0, 0, 256, 256]).div(255.0))
                if use_cuda:
                    cropped_faces = cropped_faces.cuda()
                ...
                pose_dict.update(
                    {d: [yaw_list_deg, pitch_list_deg, roll_list_deg]})
                ...
                # Predict AUs
                au_intensities = AU_detector.detect_AU(
                    cropped_faces).detach().cpu().numpy().transpose()
            ...
            for t, k in enumerate(AUs.keys()):
                np.save(osp.join(np_files_dir, '{}_{}.npy'.format(k, AUs[k])),
                        aus_np[t, :])

            # Save CelebA attributes ("Bangs", "Eyeglasses", "Beard", "Smiling", "Age") in json and numpy array format
            with open(osp.join(json_files_dir, 'celeba_bangs.json'),
                      'w') as out:
                json.dump(celeba_bangs_dict, out)
            np.save(osp.join(np_files_dir, 'celeba_bangs.npy'),
                    celeba_bangs_np)

            with open(osp.join(json_files_dir, 'celeba_eyeglasses.json'),
                      'w') as out:
                json.dump(celeba_eyeglasses_dict, out)
            np.save(osp.join(np_files_dir, 'celeba_eyeglasses.npy'),
                    celeba_eyeglasses_np)

            with open(osp.join(json_files_dir, 'celeba_beard.json'),
                      'w') as out:
                json.dump(celeba_beard_dict, out)
            np.save(osp.join(np_files_dir, 'celeba_beard.npy'),
                    celeba_beard_np)

            with open(osp.join(json_files_dir, 'celeba_smiling.json'),
                      'w') as out:
                json.dump(celeba_smiling_dict, out)
            np.save(osp.join(np_files_dir, 'celeba_smiling.npy'),
                    celeba_smiling_np)
