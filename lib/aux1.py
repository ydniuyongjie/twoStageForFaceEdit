...
class TrainingMixStatTracker(object):
    def __init__(self):
        self.stat_tracker = {
            'accuracy': [],
            'classification_loss': [],
    ...
    def update(self, accuracy, classification_loss, regression_loss, penalty_loss,total_loss):
        if accuracy is not None:
            self.stat_tracker['accuracy'].append(float(accuracy))
        else:
            self.stat_tracker.pop('accuracy','no accuracy key')
        ...
        if penalty_loss is not None:
            self.stat_tracker['penalty_loss'].append(float(penalty_loss))
        else:
            self.stat_tracker.pop('penalty_loss','no accuracy key')
...
def create_exp_dir(args,method=None):
    ...
    if method is None:
        ...
        exp_dir=method+"_{}".format(args.gan_type)
    ...
    if args.path_type=="line":
        exp_dir+="-K{}-S{}-{}".format(args.directions_count , args.shift_scale , args.min_shift)
    else:
