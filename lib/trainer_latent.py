...
class Trainer_latent(object):
    ...
    def make_shifts(self, latent_dim):
        ...
        shifts = self.params.shift_scale * shifts #self.p.shift_scale is Coefficient(系数)
    ...
    def train(self, generator, deformator, reconstructor):
        ...
        support_sets_optim = torch.optim.Adam(deformator.parameters(), lr=self.params.deformator_lr)
        ...
        support_sets_model_filename = osp.join(self.models_dir, 'deformator.pt')
        torch.save(deformator.state_dict(), support_sets_model_filename)
