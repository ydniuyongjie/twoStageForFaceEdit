...
def prepare_parser():
  ...
  ### Hessian Penalty Direction Discovery stuff ###
  parser.add_argument(
    '--G_path', type=str, default=None,
    help='Path to pre-trained BigGAN checkpoint folder (default: auto-download checkpoint)')
  parser.add_argument(
    '--A_lr', type=float, default=0.01,
    help='Learning rate for the directions')
  parser.add_argument(
    '--ndirs', type=int, default=120,
    help='Number of directions to learn')
  parser.add_argument(
    '--fix_class', type=int, default=None,
    help='If specified, restricts sampled y to always be the given ImageNet class')
  parser.add_argument(
    '--load_A', type=str, default='random',
    help='The method for initializing/loading the direction matrix. '
         'Either "random" or "coord" if you want to use a random/standard basis A, '
         'or a path to a *.pt file containing an A matrix checkpoint.'
  )
  parser.add_argument(
    '--search_space', type=str, default='all', choices=['all', 'coarse', 'mid', 'fine'],
    help='This argument controls the subspace within Z-space in which directions '
         'will be searched. Using "all" will search for directions in all of Z-space. '
         'Using "coarse" will only search for directions in the first 40 Z components. Using '
         '"mid" will search for directions in Z components 41-80, and "fine" will search for directions'
         'in Z components 81-120 (i.e., the last 40 components). Note that this assumes dim_z=120 (the default). '
         'If you use a different dimensionality for Z, arguments besides "all" will not work.'
  )
  parser.add_argument(
    '--path_size', type=float, default=5.0,
    help='Maximum possible distance to travel in the learned directions (used in both training and visualizations)'
  )
  parser.add_argument(
    '--no_ortho', action='store_true',
    help='If specified, does NOT apply Gram Schmidt to orthogonalize the A matrix.'
  )
  parser.add_argument(
    '--vis_during_training', action='store_true',
    help='If specified, periodically logs GIFs of learned directions during training to TensorBoard/WandB'
  )
  parser.add_argument(
    '--wandb_entity', type=str, default=None,
    help='To use WandB logging, supply your entity/username (if left as None, uses TensorBoard instead)'
  )
  parser.add_argument(
    '--directions_to_vis', type=int, nargs='+', default=None,
    help='If specified, only visualizes  (default: visualize all directions; used for visualizations only)'
  )
  parser.add_argument(
    '--n_samples', type=int, default=6,
    help='Number of samples to visualize per-direction (used for visualizations only)'
  )
  parser.add_argument(
    '--nz_per_vid', type=int, default=8,
    help='Number of directions to visualize per mp4 video (used for visualizations only)'
  )
  parser.add_argument(
    '--val_minibatch_size', type=int, default=80,
    help='Batch size to use when generating visuals of directions (used for visualizations only)'
  )
...
def save_weights(G, D, state_dict, weights_root, experiment_name, 
  ...
  if D is not None:  # Don't do this for fine tuning, it just wastes space.
    if name_suffix:
  ...
  if G_ema is not None and D is not None:
    torch.save(G_ema.state_dict(), 
                '%s/%s.pth' % (root, join_strings('_', ['G_ema', name_suffix])))


# Load a model's weights, optimizer, and the state_dict
def load_weights(G, D, state_dict, weights_root=None, experiment_name=None,
                 name_suffix=None, G_ema=None, strict=True, load_optim=True, direct_path=None):
