...
def main():
    """line latent Space -- Latent space traversal script.

    A script for traversing the latent space of a pre-trained GAN generator through paths defined by the Columns of the matrix. 
    Latent codes are drawn from a pre-defined collection via the `--pool` argument. 
    The generated images are stored under `results/` directory.
    ...
    Options:
        ...
        --shift-leap  : set path shift leap (after how many steps to generate images)经过多少步生成图像
        ...
        ================================================================================================================
    """
    parser = argparse.ArgumentParser(description="W Space latent space traversal script")
    ...
    parser.add_argument('--pool', type=str, required=True, help="directory of pre-defined pool of latent codes(created by `sample_gan.py`)")
    parser.add_argument('--shift-steps', type=int, default=16, help="set number of shifts per positive/negative path direction")
    ...
    parser.add_argument('--shift-leap', type=int, default=1, help="set path shift leap (after how many steps to generate images)")
    ...
    deformator_model = osp.join(models_dir, 'deformator.pt')
    if not osp.isfile(deformator_model):
        deformator_checkpoint_files = []
        for f in models_dir_files:
            if 'deformator-' in f:
                deformator_checkpoint_files.append(f)
        deformator_checkpoint_files.sort()
        deformator_model = osp.join(models_dir, deformator_checkpoint_files[-1])
    ...
    # Build deformator model D
    if args.verbose:
        print("#. Build deformator model D...")
    ...
    if args.verbose:
        print("  \\__Pre-trained weights: {}".format(deformator_model))
    D.load_state_dict(torch.load(deformator_model, map_location=lambda storage, loc: storage))
    if args.verbose:
    ...
    num_gen_paths = args_json.__dict__["directions_count"]
    ...
    for i in range(num_of_latent_codes):
        ...
        for dim in range(num_gen_paths):
            ...
            # == Positive direction == 
            cnt = 0
            for step in range(0,args.shift_steps+1):
                cnt += 1
                # Calculate shift vector based on current z
                mask = torch.zeros(1, D.input_dim)
                mask[0, dim] = step * args.eps              
                # Store latent codes and shifts
                if cnt == args.shift_leap:
                    current_path_mask.append(mask)                    
                    cnt = 0

            cnt = 1
            for step in range(1,args.shift_steps+1):
                cnt += 1
                # Calculate shift vector based on current z
                mask = torch.zeros(1,  D.input_dim)
                mask[0, dim] = -step * args.eps
                if cnt >= args.shift_leap :
                    current_path_mask = [mask] + current_path_mask
                    cnt = 0
            # ========================

            # Generate transformed images
            # Split latent codes and shifts in batches
            current_path_mask = torch.cat(current_path_mask)
            current_path_latent_shifts=D(current_path_mask)
            current_path_latent_shifts_batches = torch.split(current_path_latent_shifts, args.batch_size)

            num_batches = len(current_path_latent_shifts_batches)

            transformed_img = []
            for t in range(num_batches):
                with torch.no_grad():
                    if args_json.__dict__["shift_in_w_space"]:
                        transformed_img.append(G(z=z,
            ...
            # Append latent paths
            paths_latent_codes.append(current_path_mask.unsqueeze(0))
